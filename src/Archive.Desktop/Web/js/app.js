(function () {
  ...
  function renderBooks() {
	...
	state.books.forEach((book) => {
	  ...
	  openBtn.disabled = !(book.filePath || book.fileUrl);
	  openBtn.onclick = async () => {
		const result = await sendRequest("openFile", { id: book.id });
		if (!result.success) {
  ...
  function openEditModal(book) {
	...
	state.pendingCoverPath = null;
	state.pendingFilePath = null;
	coverFileName.textContent = book.coverUrl || book.coverPath ? "غلاف محفوظ مسبقاً" : "";
	bookFileName.textContent = book.filePath || book.fileUrl ? "ملف محفوظ مسبقاً" : "";
	bookModal.classList.remove("hidden");
  }
  ...
  async function deleteBook(id) {
	...
	if (!confirmed) return;
	const result = await sendRequest("delete", { id });
	if (!result.success) {
	  alert(result.error || "تعذر حذف الكتاب");
	  return;
	}
	await refreshBooks();
  }
  ...
  bookForm.addEventListener("submit", async (e) => {
	...
	if (payload.id) {
	  const result = await sendRequest("update", payload);
	  if (result && result.success === false) {
		alert(result.error || "تعذر تحديث الكتاب");
		return;
	  }
	} else {
	  const result = await sendRequest("add", payload);
	  if (result && result.success === false) {
		alert(result.error || "تعذر إضافة الكتاب");
		return;
	  }
	}
